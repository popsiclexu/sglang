...
def ref_sparse_mla_fwd_interface(q, kv, indices, sm_scale=None, is_casual=True):
    q = q.float()
    kv = kv.float()
    # NOTE(MUSA): JIRA: https://jira.mthreads.com/browse/KUAE-840
    ...
    mask = q.new_zeros(b, sq, g_index, sk + 1, dtype=torch.bool).scatter(3, indices.long(), 1)
    # NOTE(MUSA): JIRA: https://jira.mthreads.com/browse/KUAE-840
    ...
    assert not torch.isnan(p).any(), "NaN detected!"
...
def tilelang_sparse_fwd(
    ...
    USE_MT_TILELANG_IMPL = False
    if USE_MT_TILELANG_IMPL:
...
#     elif _is_musa:
#         # use torch impl
#         # return ref_sparse_mla_fwd_interface(q.unsqueeze(0).cpu(), kv.unsqueeze(0).cpu(), indices.unsqueeze(0).cpu()).to(q.device)
#         indices_dtype = "int32"
#         if indices.dtype == torch.int64:
#             indices_dtype = "int64"
#         kernel = sparse_mla_fwd(
...
#             indices_dtype=indices_dtype,
#         )
#         out, lse = kernel(q.unsqueeze(0), kv.unsqueeze(0), indices.unsqueeze(0))
#         return out
#     else:
