...
void musa_top_k_top_p_sampling_from_probs(
  ...
  at::PhiloxCudaState rng_engine_inputs = gen->philox_musa_state(32 * batch_size);
  philox_seed = rng_engine_inputs.seed_.val;
  philox_offset = rng_engine_inputs.offset_.val;

  const c10::cuda::OptionalCUDAGuard device_guard(device);
  auto stream = at::cuda::getCurrentCUDAStream();
  cudaError_t status = MusaTopKTopPSamplingFromProb<float, int>(
  ...
  TORCH_CHECK(
      status == cudaSuccess,
      "MusaTopKTopPSamplingFromProb failed with error code " + std::string(cudaGetErrorString(status)));
}
