        ...
        *(BVec *)(b_reg) = *(BVec *)(b_base_ptr + k_idx / w4a16_shift);
        ...
        if constexpr (is_w4a16 && !is_fp8) {
            ...
            for (int i = 0; i < thread_sum_len; i++) {
                ...
                _float_v4 b_float4;
                if constexpr (fuse_castfp8) {
                    asm("CVT.RTE.DENORM.BST4 %0.f16, %1.e4m3;" : "=R"(b_halfv4):"R"(reinterpret_cast<float*>(b_reg)[i]));
                    #pragma unroll
                    for (int j = 0; j < 4; j++) {
                        cur_thread_sum[i] += scale_val * float(a_reg[i * 4 + j]) * (b_halfv4[j]);
                    }
                } else {
                    asm("CVT.RTE.DENORM.BST4 %0.f16, %1.e4m3;" : "=R"(a_halfv4):"R"(reinterpret_cast<float*>(a_reg)[i]));
                    asm("CVT.RTE.DENORM.BST4 %0.f16, %1.e4m3;" : "=R"(b_halfv4):"R"(reinterpret_cast<float*>(b_reg)[i]));
                    #pragma unroll
                    for (int j = 0; j < 4; j++) {
